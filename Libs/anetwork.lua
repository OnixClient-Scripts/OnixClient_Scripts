    ...
    while true do
        while thread:hasMessage() do
            ...
            elseif msg.type == "post_request" then
                local response, error = nil,nil
                if msg.saveFilepath then
                    response, error = snetwork.filepost(msg.url, msg.saveFilepath, msg.body, msg.headers, msg.method, ProgressCallback, msg.files)
                else
                    response, error = snetwork.post(msg.url, msg.body, msg.headers, msg.method, ProgressCallback, msg.files)
                end
                thread:pushMessage(tableToJson({
                    type = "completed_request",
                    requestId = msg.requestId,
            ...
            elseif msg.type == "post_from_file_request" then
                local response, error = nil,nil
                if msg.saveFilepath then
                    response, error = snetwork.filepostfile(msg.url, msg.saveFilepath, msg.bodyPath, msg.headers, msg.method, ProgressCallback, msg.files)
                else
                    response, error = snetwork.postfile(msg.url, msg.bodyPath, msg.headers, msg.method, ProgressCallback, msg.files)
                end
                thread:pushMessage(tableToJson({
                    type = "completed_request",
                    requestId = msg.requestId,
...
---@field files HttpFileContent[]|nil The files to send with the request
...
---@return ANETWORK_Request
function anetwork.post(url, body, headers, doneCallback, method, files)
    assert(anetwork.internal.HasBeenInitialized, "anetwork has not been initialized")
    local request =  {
        url = url,
        headers = headers,
...
---@param files HttpFileContent[]|nil A table of files to send with the request.
---@return ANETWORK_Request
function anetwork.filepost(url, body, filepath, headers, doneCallback, method, files)
    assert(anetwork.internal.HasBeenInitialized, "anetwork has not been initialized")
    local request =  {
        url = url,
        savePath = filepath,
...
---@param files HttpFileContent[]|nil A table of files to send with the request.
---@return ANETWORK_Request
function anetwork.postfile(url, filepathToBody, headers, doneCallback, method, files)
    assert(anetwork.internal.HasBeenInitialized, "anetwork has not been initialized")
    local request =  {
        url = url,
        headers = headers,
...
---@param files HttpFileContent[]|nil A table of files to send with the request.
---@return ANETWORK_Request
function anetwork.filepostfile(url, filepathToBody, saveFilepath, headers, doneCallback, method, files)
    assert(anetwork.internal.HasBeenInitialized, "anetwork has not been initialized")
    local request =  {
        url = url,
