...
function ida3.ida3decode_synchsafe_integer(file)
...
function ida3.ida3v2_read_string_with_encoding(file, encoding)
    if encoding == 0 then
...
function ida3v2_extract_text_frame(file, segSize)
    local filepos = file:tell()
    local encoding = file:readUByte()
    local text = ida3.ida3v2_read_string_with_encoding(file, encoding)
...
function ida3.parse_ida3v2(file)
    ...
    local tag_size = ida3.ida3decode_synchsafe_integer(file)
    ...
    function tag_data:getImage(file)
        ...
        if self.image then
            ...
            local img = file:parseImage(self.image.size)
            file:seek(prevPos)
            return img
        end
        return nil
    ...
    while not file:eof() and file:tell() < tag_size do
        ...
        if frame_id == "TIT2" then
            tag_data.title = ida3v2_extract_text_frame(file, frame_size)
        elseif frame_id == "TPE1" then
            tag_data.artist = ida3v2_extract_text_frame(file, frame_size)
        elseif frame_id == "TALB" then
            tag_data.album = ida3v2_extract_text_frame(file, frame_size)
        elseif frame_id == "TYER" then
            tag_data.year = ida3v2_extract_text_frame(file, frame_size)
        elseif frame_id == "COMM" then
            local encoding = file:readUByte()
            local language = file:read(3)
            local description = ida3.ida3v2_read_string_with_encoding(file, encoding)
            local comment = ida3.ida3v2_read_string_with_encoding(file, encoding)
        ...
        elseif frame_id == "APIC" then
            ...
            local pic_desc = ida3.ida3v2_read_string_with_encoding(file, encoding)
...
---@param file BinaryFile
---@return TitleInformation|nil info
function ida3.parse_ida3v1(file)
...
function ida3.parse(file)
    if file == nil then return nil end
    if type(file) == "string" then
        file = fs.open(file, "r")
    end
    if file == nil then return nil end
    local fileInfo = ida3.parse_ida3v2(file)
        if fileInfo == nil then
            fileInfo = ida3.parse_ida3v1(file)
        end
        if fileInfo == nil then
            file:close()
            return nil
