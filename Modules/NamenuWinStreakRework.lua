...
function gmSelector(msg)
    ...
    if file then
        -- Variables pour la ligne à lire
        local lineNumber = 0
        local newWinstreak = nil  -- Nouvelle valeur de winstreak
        for line in file:lines() do
            lineNumber = lineNumber + 1
            -- Vérifier quel mode de jeu est sélectionné
...
function onChat(message, username, type)
    ...
    if string.match(message, team .. " Team §7has been §cELIMINATED§7!") then
        ...
        if string.match(message, player.name()) then
            -- Reset the winstreak to 0 on loss
            winstreak = 0
    ...
    -- Case for winning message for team
    if string.match(message, team .. " Team are the WINNERS!") then
        if string.match(message, player.name()) then
            -- Increase the winstreak if the player is part of the winning team
...
function updateWinstreakInFile(lineNumber, newWinstreak)
 ...
 function setstreak(arg)
    -- Ensure that the argument is a valid number (in case you want to prevent invalid input)
    if type(arg) == "number" then
        winstreak = arg  -- Set winstreak to the provided argument
        updateWinstreakInFile(NumberLine, winstreak)  -- Update the winstreak in the file
    else
...
registerCommand("addstreak", function()
    -- Get the current winstreak (assuming you have a global variable `winstreak` defined)
    winstreak = winstreak + 1  -- Increment the winstreak by 1
    
    -- Update the winstreak in the file
    updateWinstreakInFile(NumberLine, winstreak)
    
    -- Print the updated winstreak to the console
