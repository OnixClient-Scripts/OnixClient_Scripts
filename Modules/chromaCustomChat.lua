name="chroma custom chat"
description = "Chroma custom chat by MCBE Craft"
...
function render(deltaTime)
        ...
        for i = 0, 8, 1 do
            if chat[i] ~= nil then
                if chat[i + chatMessagePos][1] ~= "" then
                    ...
                    table.insert(texts, {text=chat[i + chatMessagePos][1] .. ": " .. chat[i + chatMessagePos][2], x=5, y=10*i - 8})
                    -- chromaText(5, 10*i - 8, chat[i + chatMessagePos][1] .. ": " .. chat[i + chatMessagePos][2], math.floor(textColor.a * (fading/fadeDefault)))
                else
                    --gfx.text(5, 10*i - 8, chat[i + chatMessagePos][2])
                    table.insert(texts, {text=chat[i + chatMessagePos][2], x=5, y=10*i - 8})
                    -- chromaText(5, 10*i - 8, chat[i + chatMessagePos][2], math.floor(textColor.a * (fading/fadeDefault)))
                end
            end
        end
    end
    if writeShow then
        ...
        -- chromaText(5, 89, toSend, textColor.a)
        table.insert(texts, {text=toSend, x=5, y=89})
    ...
    chromaTextList(0, 0, texts, math.floor(textColor.a * (fading/fadeDefault)))
end
        ...
        if holdCtrl then
            ...
            if holdShift then
                ...
                if cursorPos < #toSend or selectionPos < #toSend then
                    cursorPos = math.max(cursorPos, selectionPos) + 1
                    selectionPos = cursorPos
                end
            end
        end
