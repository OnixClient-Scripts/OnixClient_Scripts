...
function render(deltaTime)
    ...
    local durability = 0
    local ores = {"iron", "gold", "copper", "redstone", "lapis", "emerald", "quartz", "diamond"}
    local amountOre = {0, 0, 0, 0, 0, 0, 0, 0}
    local amountRessource = {0, 0, 0, 0, 0, 0, 0, 0}
    local pathOre = {"", "", "", "", "", "", "", ""}
    local pathRessource = {"", "", "", "", "", "", "", ""}
    for i=1,inventory.size do
        ...
        if (slot ~= nil) then
            ...
            elseif (string.find(slot.name, "_ore") ~= nil) then
                for a=1,8 do
                    local ore = ores[a]
                    if (string.find(slot.name, ore)) then
                ...
                for a=1,8 do
                    local ore = ores[a]
                    if (slot.name == ore or slot.name == ore .. "_ingot" or slot.name == "raw_" .. ore or slot.name == ore .. "_lazuli") then
